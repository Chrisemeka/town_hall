...
export function TestResultCard({ data }: { data: any }) {
  ...
  return (
    <div className="group bg-surface rounded-2xl overflow-hidden border border-outline-variant hover:border-outline transition-all shadow-sm flex flex-col md:flex-row">
      {/* Left side: Image */}
      <div className="w-full md:w-2/5 lg:w-1/3 aspect-video md:aspect-auto relative bg-surface-variant overflow-hidden cursor-pointer border-b md:border-b-0 md:border-r border-outline-variant shrink-0" onClick={() => setIsOpen(true)}>
        <img 
        ...
        <div className={`absolute top-4 left-4 px-3 py-1.5 rounded text-xs font-semibold tracking-wide uppercase shadow-sm border backdrop-blur-md
          ...
          {data.ai_sentiment || "NEUTRAL"}
        </div>

        <div className="absolute bottom-4 left-4 w-8 h-8 rounded bg-surface/80 backdrop-blur-md border border-outline-variant flex items-center justify-center text-on-surface shadow-sm opacity-0 group-hover:opacity-100 transition-opacity">
          <Maximize2 size={14} />
        </div>
      </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-surface/95 backdrop-blur-sm p-4" onClick={() => setIsOpen(false)}>
          ...
          <div className="bg-surface border border-outline-variant shadow-2xl rounded-2xl p-2 max-w-5xl w-full">
            <img 
              src={data.screenshot_url} 
